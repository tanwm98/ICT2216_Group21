...
const {
    isBreachedPassword
} = require('../middleware/breachCheck');
...
router.get('/reservations', async (req, res) => {
    ...
    try {
        const reservations = await db('reservations as r')
            ...
            .orderBy([{
                    column: 'r.reservationDate',
                    order: 'desc'
                },
                {
                    column: 'r.reservationTime',
                    order: 'desc'
                }
            ]);
...
router.post('/reset-password', userPasswordValidator, handleValidation, async (req, res) => {
    ...
    try {
        user = await db('users')
            .select('password', 'refresh_token_version')
            .where('user_id', userId)
            .first();
        ...
        if (!user) {
            return res.status(401).json({
                error: 'Error performing password reset.'
            });
        }
    } catch (error) {
...
router.put('/edit/firstname', userFirstNameValidator, handleValidation, async (req, res) => {
    ...
    if (/\d/.test(firstname)) {
        return res.status(400).json({
            message: 'First name cannot contain numbers and characters.'
        });
    }
...
router.put('/edit/lastname', userLastNameValidator, handleValidation, async (req, res) => {
    ...
    if (/\d/.test(lastname)) {
        return res.status(400).json({
            message: 'Last name cannot contain numbers and characters.'
        });
    }
...
router.get('/reservations/:id/edit', async (req, res) => {
    ...
    try {
        ...
        if (now >= reservationDateTime) {
            return res.status(400).json({
                error: 'Cannot edit a past or ongoing reservation'
            });
        }
